	...
	for (unsigned long long i = 5; i < MAX; i++) { ////////////////////////////////////////////////////////////////////////////////
        if (!BITTEST(arr, i)){ // This could cost less mem. lets say !BITTEST(arr, i) if 2*i+1 is prime. then 2 bitsets and 6*i+1,6*i-1. Wheels!
	    	...
	    	unsigned int upper_bound = (MAX2)/2 + 1;
    	for (unsigned int i = 2; i < upper_bound; ++i) {
    		if (factors[i] == 0) {
    			for (unsigned int j = 2*i; j < MAX2; j += i) {
    				factors[j] = i;
    			}
    		}
    	}
		for (unsigned int i = 2; i < MAX2; ++i) {
			if (factors[i] == 0){
				factors[i] = i;
			}
		}
    ...
    printf("Finding numbers %hhu %hhu %hhu \n",gaps[1],gaps[2],gaps[3]);
    	for (unsigned long long int i = start; i < MAX - 1; ++i) {
    		if (!BITTEST(arr, i+1)) {
					...
					while (number>MAX2-10){
						if (number%index==0){
							...
							if (v(index, i*2) < 2*v(index, i+1)){
								lat =-1;
								happenedTwo=true;break;
							}
							if (number==1){happenedTwo=true;break;}
							if (!BITTEST(arr, number)){
								if (v(number, i*2) < 2*v(number, i+1)){
								lat =-1;
								}
				...
				if (!happenedTwo){
					while (number!=1){
						unsigned int f= factors[number];
						while (number%f==0){
								number/=f;
						}
						if (v(f, i*2) < 2*v(f, i+1)){
								lat =-1;
								break;
							}
					}
